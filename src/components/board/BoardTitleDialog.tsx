...
interface Props {
  deleted?: boolean; heading: string; initial?: string; pending: boolean; error: string | null; register: RegisterDraftGuard
  onSave: (title: string) => Promise<boolean>; onClose: () => void
}
export function BoardTitleDialog({ heading, initial = '', pending, deleted, error, register, onSave, onClose }: Props) {
  ...
  return <BoardDialog title={heading} onClose={close}>
    ...
    <form onSubmit={async (event) => {
      ...
      if (!deleted && await onSave(title.trim())) onClose()
    }}>
      <fieldset disabled={pending}>
        <label>Title<input required maxLength={500} readOnly={deleted} value={title}
          onChange={(event) => setTitle(event.target.value)} /></label>
        <div className="kanban-actions">
          <button type="submit" className="kanban-primary" disabled={deleted || !title.trim()}>Save</button>
