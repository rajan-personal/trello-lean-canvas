...
import { sectionTemplate, type SectionId } from '../src/data'
...
function makeCanvasYaml({ name, title = name, cards = {} }: CanvasYamlInput): string {
  return dump({
    canvas: {
      name,
      title,
...
async function uploadCanvas(page: Page, canvas: CanvasYamlInput, fileName = 'canvas.yaml'): Promise<void> {
...
async function openSampleCanvas(page: Page): Promise<void> {
  await page.goto('/')
  await uploadCanvas(page, { name: 'Team alignment', title: 'Pulse', cards: sampleCards }, 'team-alignment.yaml')
  await expect(page.getByRole('heading', { name: 'Pulse' })).toBeVisible()
}

test('starts with researched examples, creates, uploads, edits, switches, and exports independent canvases', async ({ page }) => {
  await page.goto('/')
  await expect(page.getByRole('heading', { name: 'Airbnb — 2008' })).toBeVisible()
  await expect(page.getByRole('navigation', { name: 'Lean canvases' }).getByRole('button')).toHaveCount(4)
...
test('reorders sidebar canvases with drag and drop and persists the order', async ({ page }) => {
  await page.goto('/')

  const navigation = page.getByRole('navigation', { name: 'Lean canvases' })
  const airbnb = navigation.getByRole('button', { name: 'Airbnb', exact: true })
  const facebook = navigation.getByRole('button', { name: 'Facebook', exact: true })
  ...
  await expect(navigation.getByRole('button').nth(0)).toHaveAccessibleName('Facebook')
  await expect(navigation.getByRole('button').nth(1)).toHaveAccessibleName('Airbnb')
...
test('moves a focused sidebar canvas with Alt and arrow keys', async ({ page }) => {
  await page.goto('/')

  const navigation = page.getByRole('navigation', { name: 'Lean canvases' })
  await navigation.getByRole('button', { name: 'Airbnb', exact: true }).press('Alt+ArrowDown')
...
test('loads fresh sample data without removing custom canvases or creating duplicates', async ({ page }) => {
  ...
  await expect(page.getByRole('navigation', { name: 'Lean canvases' }).getByRole('button')).toHaveCount(5)
  await expect(page.getByRole('button', { name: 'My startup', exact: true })).toBeVisible()
...
test('keeps the white canvas columns coherent and evenly sized', async ({ page }) => {
  await page.setViewportSize({ width: 1424, height: 797 })
  await openSampleCanvas(page)

  const columnHeights = await page.locator('.canvas-column').evaluateAll((columns) => (
    columns.map((column) => Math.round(column.getBoundingClientRect().height))
  ))
  const secondRowTops = await page.locator('.canvas-column').evaluateAll((columns) => (
    columns.map((column) => Math.round(column.children[1].getBoundingClientRect().top))
  ))
  const bottomPanelHeights = await page.locator('.bottom-panel').evaluateAll((panels) => (
...
test('expands the white column for the composer without nested scrolling', async ({ page }) => {
  ...
  const layout = await section.evaluate((element) => {
    ...
    if (!composer || !element.parentElement) throw new Error('Composer layout is incomplete')
...
test('keeps the first card anchored when it changes from composer to saved card', async ({ page }) => {
  ...
  const emptyMainCellHeights = await page.locator('.canvas-column .canvas-cell').evaluateAll((cells) => (
    cells.map((cell) => Math.round(cell.getBoundingClientRect().height))
  ))
  const emptyBottomCellHeights = await page.locator('.bottom-panel .canvas-cell').evaluateAll((cells) => (
    cells.map((cell) => Math.round(cell.getBoundingClientRect().height))
  ))
