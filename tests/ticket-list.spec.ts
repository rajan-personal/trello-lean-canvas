...
test('tracks sidebar order through status controls and project reordering', async ({ page }) => {
  ...
  await expect(ticketRows(page)).toHaveCount(5)
...
test('fills the available height without project tags or filter menus', async ({ page }) => {
  await openList(page)
  ...
  await expect(page.getByRole('table').getByRole('searchbox')).toHaveCount(0)
  await page.getByRole('button', { name: /Sort Status/ }).click()
  await expect(ticketRows(page)).toHaveCount(5)
  await expect(page.getByRole('group', { name: /Filter/ })).toHaveCount(0)
  await openList(page, { projects: [projects[2]], boards: { c: boards.c } })
  expect(await ticketRows(page).count()).toBe(0)
  const area = await page.locator('.ticket-list-area').boundingBox(); const shell = await page.locator('.ticket-list-table-shell').boundingBox(); expect(shell!.height).toBeGreaterThan(area!.height - 60)
})
test('keeps ticket deep links, project-board access, and history behavior', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'All tickets', exact: true }).click(); await expect(page).toHaveURL('/tickets'); await page.locator('.ticket-list-ticket-row[data-ticket-id="a-duplicate-first"] .ticket-list-ticket-button').click()
...
test('honors the dirty ticket guard when leaving a project board for the list', async ({ page }) => {
  ...
  page.once('dialog', (dialog) => dialog.dismiss()); await page.getByRole('button', { name: 'All tickets', exact: true }).click(); await expect(page).toHaveURL('/project/a/ticket'); await expect(page.getByRole('textbox', { name: 'Card title', exact: true })).toHaveValue('Keep this draft') })
