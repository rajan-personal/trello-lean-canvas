...
test('round trips UI-created cards, ordering, description and comments through YAML into an independent canvas', async ({ page }) => {
  ...
  await expect(page.getByRole('navigation', { name: 'Workspace navigation' }).locator('.canvas-nav-item')).toHaveCount(2)
  ...
  await page.getByRole('navigation', { name: 'Workspace navigation' }).locator('.canvas-nav-item').first().click()
  await expect(column(page, 'Review').locator('.kanban-card')).toHaveText('Exported card')
})
